...
export const TreeChart = (props) => {
  ...
  function drawTreeChart(
    ...
    const svg = d3.select("#treeChart").attr("width", width).attr("height", height);
  ...
  const draw = async (data) => {
    const chart = drawTreeChart(data, {});
  };
  ...
  return (
    <div>
      <svg id="treeChart" style={{ width: windowWidth, height: windowHeight }}></svg>
    </div>
  );
};
