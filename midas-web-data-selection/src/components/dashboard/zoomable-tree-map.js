...
export const ZoomTreeMap = ({ data }) => {
  ...
  const ref = useD3(
    (svg) => {
      ...
      function render(group, root) {
        ...
        node
          .append("rect")
          .attr("id", (d) => (d.leafUid = faker.random.numeric()))
        ...
        node
          ...
          .selectAll("tspan")
          .data((d) =>
            (d === root ? name(d) : d.data.name).split(/(?=[A-Z][^A-Z])/g).concat(format(d.value))
          )
          .join("tspan")
          .attr("x", 3)
  ...
  return <svg ref={ref} id="zoom-map" style={{ width: "100%", height: "100%" }}></svg>;
};
