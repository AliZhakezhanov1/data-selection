...
export const StackBarNormalize = ({ data }) => {
  // const ages = data.columns.slice(1)
  const ages = ["happy", "sad", "anger", "excitement"];
  const stateages = ages.flatMap((age) =>
    data.map((d) => ({ state: d.name, age, population: d[age] }))
  ); // pivot longer
  function StackedBarChart(
    data,
    {
      ...
      height = 620, // outer height, in pixels
    ...
    svg
      ...
      .call((g) =>
        g
          ...
          .attr("text-anchor", "end").attr("font-size", 15)
          .text(xLabel)
      );
