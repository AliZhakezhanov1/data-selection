...
export const BarChartTransition = ({ data  , labely}) => {
  ...
  function BarChart(
    data,
    {
      ...
      height = 640, // the outer height of the chart, in pixels
      ...
      update(
        ...
        rect = rect
          ...
          .join(
            ...
            (update) => update,
            (exit) =>
              exit.transition(t).delay(delay).attr("y", yScale(0)).attr("height", 0).remove()
          );
...
//   }
  ...
  const draw = async (data) => {
    ...
    // const update = chart.update(d3.sort(data, trigger))
  };
  useEffect(() => {
    draw(data);
  },[data.length]);
